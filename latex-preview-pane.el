;;; latex-preview-pane.el --- provides an updatable preview pane for editing
;;; LaTeX in Emacs.

;; Copyright (C) 2020 Maxime Robeyns <maximerobeyns@gmail.com>
;; Copyright (C) 2015 John L. Singleton <jsinglet@gmail.com>

;; This is a fork of latex-preview-pane; the original can be found at
...
;; LaTeX files directly in Emacs.
;; It supports PDF previews, your choice of pdflatex or xelatex, and it
;; highlights errors in your LaTeX buffer.
;;
...
;;
;; The latest version of this fork can be found at the following repository
;; https://github.com/maximerobeyns/latex-preview-pane
;;
;; You can find (outdated) documentation either on GitHub (above) or
...
(defvar latex-preview-pane-current-version "20200712")
...
(defun lpp/window-containing-preview ()
  "Finds the window containing the current preview"
...
"
  (progn
    ...
    ;; refresh that pane
    (run-at-time "0 min 1 sec" nil 'latex-preview-pane-update)))

(defun lpp/get-message (f)
  "Get the string contents of file 'f'"
  (with-temp-buffer
...
(defun latex-preview-update ()
  "Updates an external program with current file."
  (interactive)
  (let ((pdf-file
         (replace-regexp-in-string "\.tex$" ".pdf" (lpp/buffer-file-name))))
...
;; TODO make these colours less aggressive
(defface bad-face
  '((t (:foreground "White" :background "#ff0033")))
  "Face for errors"
  ...
  (let ((old-buff (current-buffer)))
    ...
    (let ((error-msg (buffer-substring (point-min) (point-max))))
      ...
      (mapcar
       ...
       (latex-pp-filter
        (lambda (what) (eq (string-match "l\\.[0-9]*" what) 0))
...
(defun lpp/line-errors-to-layovers (errors)
  (mapcar (lambda (what) (let ( (line (string-to-number what)))
                           (let (layoverStart layoverEnd)
                             ...
                             ;; create the layover
                             (overlay-put
                              (make-overlay layoverStart layoverEnd)
                              'face 'bad-face))))
          errors))
...
(defun lpp/is-tex (string)
  (and (string-match (rx-to-string `(: ,".tex" eos) t) string)
       t))

(defun lpp/auctex-buffer-file-name ()
  (cond
   ((eq (boundp 'TeX-master) nil)
    (message "The TeX master variable is not defined. To use this mode you must be using AUCTeX on this buffer."))
   ((eq TeX-master nil)
...
(defun latex-preview-pane-update-p ()
  "Runs the re-compilation process"
  (if (eq (lpp/invoke-pdf-latex-command) 1)
      ;; if pdflatex exited with an error:
      (progn
        (lpp/display-backtrace)
        (remove-overlays)
        (lpp/line-errors-to-layovers (lpp/line-errors)))

    ;; otherwise the compilation happened successfully
...
;; TODO update this to include a background-compile mode
...
(easy-menu-define words-menu latex-preview-pane-mode-map
       ...
       '("LaTeX Preview Pane"
         ...
         ["Multi-File Mode" nil
          :active nil]
         ["Off" (lpp/set-multifile-mode 'off)
          :style radio
          :selected (eq latex-preview-pane-multifile-mode 'off)]
         ["Use AUCTeX/TeX-master" (lpp/set-multifile-mode 'auctex)
          :style radio
          :selected (eq latex-preview-pane-multifile-mode 'auctex)]
         ["Prompt" (lpp/set-multifile-mode 'prompt)
          :style radio
...
(let ((installation-dir
  ...
  (defvar message-latex-preview-pane-welcome
    (lpp/get-message
     (expand-file-name "message-latex-preview-pane-welcome.txt" installation-dir)))
  (defvar message-no-preview-yet
    (lpp/get-message
...
(defcustom use-latexmk nil
  "Should we use latexmk instead of pdflatex?"
  :type 'boolean
  :group 'latex-preview-pane)
